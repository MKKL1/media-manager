...
const backendMetricTag = "backend"

func newSubWorkflowFailedError(err error) *history.SubWorkflowFailedAttributes {
	...
	attrs := &history.SubWorkflowFailedAttributes{}
...
func NewPostgresBackend(host string, port int, user, password, database string, opts ...option) *postgresBackend {
	...
	if options.EnableNotifications {
		notifyDSN := dsn
		if options.NotificationDSN != "" {
			notifyDSN = options.NotificationDSN
		}
		b.listener = newNotificationListener(notifyDSN, options.Logger, options.NotificationFallbackTimeout)
	}
...
func NewPostgresBackendWithDB(db *sql.DB, opts ...option) *postgresBackend {
	...
	if options.EnableNotifications && options.NotificationDSN == "" {
		panic("postgres: WithNotifications(true) requires WithNotificationDSN() when using NewPostgresBackendWithDB")
	}
	...
	if options.EnableNotifications {
		b.listener = newNotificationListener(options.NotificationDSN, options.Logger, options.NotificationFallbackTimeout)
	}
	...
	if pb.dsn != "" {
		...
		defer func() {
			if cErr := db.Close(); cErr != nil {
				pb.options.Logger.Error("closing migration database", "error", cErr)
			}
		}()
	} else {
		db = pb.db
...
func (pb *postgresBackend) Metrics() metrics.Client {
	return pb.options.Metrics.WithTags(metrics.Tags{backendMetricTag: "postgres"})
}
...
func (pb *postgresBackend) RemoveWorkflowInstances(ctx context.Context, options ...backend.RemovalOption) error {
	...
	defer func() {
		if cErr := rows.Close(); cErr != nil {
			pb.options.Logger.Error("closing rows", "error", cErr)
		}
	}()
...
func (pb *postgresBackend) GetWorkflowInstanceHistory(ctx context.Context, instance *workflow.Instance, lastSequenceID *int64) (_ []*history.Event, retErr error) {
	tx, err := pb.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, err
	}
	...
	defer func() {
		if cErr := historyEvents.Close(); cErr != nil && retErr == nil {
			retErr = cErr
		}
	}()
...
func createInstance(ctx context.Context, tx *sql.Tx, queue workflow.Queue, wfi *workflow.Instance, m *workflow.Metadata) error {
...
func (pb *postgresBackend) GetWorkflowTask(ctx context.Context, queues []workflow.Queue) (*backend.WorkflowTask, error) {
	...
	if pb.listener != nil {
		wait := pb.listener.PrepareWaitForWorkflowTask()
...
func (pb *postgresBackend) getWorkflowTaskImpl(ctx context.Context, queues []workflow.Queue) (_ *backend.WorkflowTask, retErr error) {
	tx, err := pb.db.BeginTx(ctx, &sql.TxOptions{
		Isolation: sql.LevelReadCommitted,
	})
	if err != nil {
	...
	res, err := tx.ExecContext(
		ctx,
		`UPDATE instances i SET locked_until = $1, worker = $2 WHERE id = $3`,
	...
	var wfMeta *metadata.WorkflowMetadata
	if metadataJson.Valid {
		if err := json.Unmarshal([]byte(metadataJson.String), &wfMeta); err != nil {
	...
	defer func() {
		if cErr := events.Close(); cErr != nil && retErr == nil {
			retErr = cErr
		}
	}()
	...
	if err := row.Scan(&lastSequenceID); err != nil {
				...
				if errors.Is(err, backend.ErrInstanceAlreadyExists) {
					if err := insertPendingEvents(ctx, tx, instance, []*history.Event{
						history.NewPendingEvent(
							...
							newSubWorkflowFailedError(backend.ErrInstanceAlreadyExists),
...
func (pb *postgresBackend) GetActivityTask(ctx context.Context, queues []workflow.Queue) (*backend.ActivityTask, error) {
	...
	if pb.listener != nil {
		wait := pb.listener.PrepareWaitForActivityTask()
