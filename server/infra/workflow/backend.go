...
import (
	"database/sql"
	"log/slog"
	"server/infra/workflow/postgres"
	"time"
...
func NewBackend(host string, port int, user, password, database string, logger zerolog.Logger) diag.Backend {
	return postgres.NewPostgresBackend(host, port, user, password, database,
		postgres.WithBackendOptions(
			backend.WithLogger(slog.New(slogzerolog.Option{Logger: &logger}.NewZerologHandler())),
		),
		...
		// Enable reactive task polling via LISTEN/NOTIFY:
